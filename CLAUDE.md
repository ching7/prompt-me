...
**阶段① 基本收尾**（subagent 驱动 + 两段式审查，**89 个测试全绿**）：捕获→收件箱→今日→福格闭环→番茄钟→正反馈积分→AI/MAP/复盘 全链路打通,自测清单见 `docs/test/2026-06-10-v1-phase1-final-selftest.md`。**正反馈积分**(捕获+2/完成+10/番茄+10，**事件派生**=`TaskEventType{done,tooHard,capture,tomato,tomatoAbort}` + `ScoreCalculator`；`StatsChip ★总积分` + 庆祝 `+N 分`)。其后:
...
- **收尾**:番茄**预估入口**(专注屏 1–4 选 `setTomatoEst`，卡显 N/M)·**放弃番茄**记 `tomatoAbort`(不加分)·待办屏**「AI 整理今日」**(`prioritize` 接屏，门控 isActive)。

另含 **web 适配**(Drift WASM，`lib/data/connection/` 条件导入，`web/` 资源随仓库；作者用 Chrome 网页自测)。TDD 计划在 `docs/superpowers/plans/2026-06-*.md`。**待做**(留后续/阶段②)：复盘日历切换/历史某天 · 积分番茄曲线 · AI 结果缓存 · AI 估🍅 · 专注时长二次喂诊断；**阶段②** 桌面零摩擦捕获 + ntfy 单向同步。
