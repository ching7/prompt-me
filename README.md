...
| **A** 能力 | 任务拆解；`[太难了]` 触发**无底线降级**到 2 分钟微习惯 |
| **M** 动机 | `[我做到了]` 即时庆祝动画 + 连续天数 / 完成率 |
| **B** 调试 | AI 复盘未完成任务：定位 M/A/P 哪一项塌了，并给出修法 |

> **点睛交互 —— 卡住时不硬撑，而是变小：**
> 点 `[太难了]` → 选原因（忘记 / 太累 / 没动力，分别对应 P / A / M）→ AI 把任务「无底线降级」成一个 2 分钟、几乎不可能失败的微习惯，并刷到锁屏。降低门槛不是放弃，是福格法则。

## ✨ 功能

**✅ 已实现（V0 · 第一波）**
- 🎯 **四象限今日时间线**：苹果日历事件 + 飞书任务，按艾森豪威尔四象限聚合，「重要紧急」置顶
- 🔁 **福格闭环**：`[我做到了]`（彩纸庆祝 + 连续天数）· `[太难了]`（选原因 → 无底线降级成 2 分钟微习惯）
- 📊 连续天数 / 今日完成率
- 🧩 纯函数核心（全 TDD）：飞书 Markdown 解析、ICS 解析、福格聚合、降级、AI 提示与容错解析
- 💾 本地优先存储（SQLite / Drift）

**🚧 规划中**
- [ ] 苹果日历 **webcal 订阅**（只读、自动刷新）
- [ ] 飞书文档导入（粘贴 Markdown → 第二波 OAuth 自动同步）
- [ ] **AI 教练**：一键「整理今日」优先级建议、傍晚「复盘未完成」
- [ ] 本地通知 + 锁屏微习惯提示
- [ ] 跨端（macOS / Web）
...
| AI | 自带 Key 直连，可切换 Claude / OpenAI 兼容（DeepSeek/Qwen 等） |
...
- [x] **第一波 ②** 今日时间线 + 福格闭环 UI
- [ ] **第一波 ③** 集成：苹果日历订阅 · 飞书粘贴导入 · AI 整理/复盘 · 通知 · 设置
