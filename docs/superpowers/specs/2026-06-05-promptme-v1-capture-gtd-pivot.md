...
- 「待办」屏:GTD 智能清单(今日待办 / 下一步行动 / 即将到来 / 将来也许)+ **领域色标 + 面包屑(为什么做)+ 情境标签**;逾期态;快速录入 FAB。
...
- 每条任务:**领域色标**(接替四象限:工作 / 自媒体 / 学习 / 家庭…)+ **面包屑**(关键结果 › 执行任务 = 为什么做)+ **情境标签**(GTD 经典情境 @电脑 / @外出 / @电话 / 高精力 / 低精力 / 等待,默认 + 可自定义);内容精简(标题 + 一行面包屑 + 小标签)。
...
- **待办屏新增字段**(实现阶段细化,MVP 从简):
  - `领域 domain`、`情境标签 tags`(默认集 + 自定义):新增轻量字段 / 关联表;
...
1. **阶段 1 · 手机重构(本设计先做这步)**
   删日历 / 飞书 / 四象限 → 3 Tab IA(收件箱 / 待办 / 复盘)+ 收件箱(先支持手机 `+` 手记)+ 待办屏(GTD 智能清单 + 领域 / 面包屑 / 情境标签 + 逾期态 + FAB + 总积分 chip)+ 保留福格闭环 / AI。**不依赖桌面,自己能跑通**,可独立验证「执行 > 规划」。AI 提示横幅 / 异步分析可在阶段 1 末或紧接其后接入(client 已具备)。
2. **阶段 2 · 桌面捕获 + ntfy**
